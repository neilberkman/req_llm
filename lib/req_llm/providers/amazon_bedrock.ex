defmodule ReqLLM.Providers.AmazonBedrock do
  ...
  **Impact:** Tests or production code using slow models with high token limits may intermittently
  ...
  def attach_stream(model, context, opts, _finch_name) do
    ...
    # Get model ID - use model.model which preserves requested ID (including inference profile prefix)
    model_id = model.model
  ...
  @impl ReqLLM.Provider
  def api_model_id(model_id) when is_binary(model_id) do
    # Add global prefix for Anthropic models that require inference profiles
    # AWS Bedrock requires newer Anthropic models to use inference profile IDs
    # (e.g., "anthropic.claude-haiku-4-5-20251001-v1:0" -> "global.anthropic.claude-haiku-4-5-20251001-v1:0")
    case model_id do
      "anthropic." <> _ ->
        # Check if it already has a region prefix
        case String.split(model_id, ".", parts: 3) do
          [possible_region, "anthropic", _rest]
          when possible_region in ["us", "eu", "ap", "ca", "global"] ->
            # Already has region prefix, use as-is
            model_id

          _ ->
            # Add global prefix for Anthropic models
            "global.#{model_id}"
        end

      _ ->
        # Non-Anthropic models use model ID as-is
        model_id
    end
  end
