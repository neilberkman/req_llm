...
run_example "Text Generation" \
  mix run scripts/text_generate.exs \
  "Explain recursion in one sentence" \
  --log-level warning

run_example "Text Streaming" \
  mix run scripts/text_stream.exs \
  "Count from 1 to 5" \
  --log-level warning
...
run_example "Embeddings Single" \
  mix run scripts/embeddings_single.exs \
...
run_example "Context Reuse" \
  mix run scripts/context_reuse.exs \
  --log-level warning

run_example "Context Cross Model" \
  mix run scripts/context_cross_model.exs \
  --log-level warning
