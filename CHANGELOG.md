...
- Upgraded ex_aws_auth dependency from ~> 1.0 to ~> 1.1
- Simplified AWS session token handling in Bedrock provider
  - Session tokens now passed directly to ex_aws_auth signing functions
  - Removed manual x-amz-security-token header injection (now handled by ex_aws_auth)
